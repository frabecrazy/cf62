...
def show_main():
    st.markdown("""
        <style>
            .device-box {
                ...
                /* Rimosso il bordo sinistro azzurro */
            }
            .device-header {
                font-size: 1.4em;
                font-weight: 600;
    ...
    if st.button("➕ Add Device"):
        ...
        st.session_state.device_list.insert(0, new_id)  # Inserisce in cima
    ...
    for device_id in st.session_state.device_list:
        ...
        with st.expander(f"{base_device}", expanded=True):
...
""", unsafe_allow_html=True)
    ...
    for i, (act, ef) in enumerate(activity_factors[role].items()):
        with (col1 if i % 2 == 0 else col2):
            ore = st.slider(f"{act} (h/day)", 0.0, 8.0, 0.0, 0.5, key=act)
            ore_dict[act] = ore
    ...
    email_plain = st.selectbox("Emails sent/received during a typical 8-hour day **no attachments** - do not include spam emails", ["1–10", "11–20", "21–30", "31–40", "> 40"])
    email_attach = st.selectbox("Emails sent/received during a typical 8-hour day **with attachments** - do not include spam emails", ["1–10", "11–20", "21–30", "31–40", "> 40"])
